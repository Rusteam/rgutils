...
def calc_roc_auc(a_p, p, a_n, n):
    ...
    norms = np.concatenate((p_norms, n_norms), axis=0)
    y_true = np.concatenate((np.zeros(len(p_norms)), np.ones(len(n_norms))), axis=0)
    return roc_auc_score(y_true, norms )
